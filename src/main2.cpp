...
struct QueryOp
{
    ...
    QueryOp() {}
    QueryOp(int _end, int _ts): end(_end), timestamp(_ts) {}
};
QueryOp query_list[M];
int query_cnt = 0;

char str[QUERY_DOC_LEN];
int str_cnt;

const int TRIE_TREE_NUM = 40;

int hash_cnt[TRIE_TREE_NUM];
...
{
    // TrieTree& cur_tree = trie_tree[tree_idx];
...
int ReadBatchOperation()
{
    ...
    while(getdelim(&read_buffer, &max_line_cnt, 'F', stdin) != -1) {
        ...
        while(true) {
            ...
            if (ch == 'D' || ch == 'A') {
                ...
                *(current_pos + length) = '\0';
                ...
                memcpy(str + str_cnt, current_start + 1, length + 1);
                str_cnt += length + 1;
                str[str_cnt++] = ' ';
                str[str_cnt++] = '\0';
                query_list[query_cnt++] = QueryOp(str_cnt - 1, timestamp);
                timestamp++;       
            } 
...
void split_into_block(int len)
{
    int block_size = BLOCK_SIZE, cur = 1, query_list_idx = 0;
    while (len / block_size + 1 > MAX_BLOCK_NUM) block_size *= 2;
    block_interval_cnt = 0;
    while (cur < len) {
        int next = min(cur + block_size, len);
        if ((next - cur) * 4 < block_size && block_interval_cnt > 0) {
...
// inline bool isMatchCondEndNode(char* query_str, char* gram_str)
// {
//     while (*gram_str) {
//         if (*query_str != *gram_str) return false;
//         query_str++;
//         gram_str++;
//     }
//     return (*query_str == ' ');
...
void query_in_block(int block_idx)
{
    for (int k = block_interval[block_idx].first; k < block_interval[block_idx].last; ++ k) {
        ...
        int j = k;
        if (str[k + 1] != ' ') {
            current = trie_entry[(unsigned char)str[k]][(unsigned char)str[k + 1]];
            j += 2;
        } else {
            int tree_idx = calc_tree_idx(str + k);
        ...
        while (true) {
            ...
            if ((current->node_state) & 2) { // pass condense node.
                ...
                if (!isMatchCondNode(str + j, current->cond_str, cond_len))
                    break;
            }
...
void Process()
{
    ...
    while(true) {
        ...
        matches[match_cnt++] = Match(str_cnt, str_cnt, NULL);
